...
public class TipoDocumentoTableModel extends AbstractTableModel {
    ...
    public TipoDocumentoTableModel() {
        list = new LinkedList<TipoDocumento>();
    }

    public TipoDocumentoTableModel(List<TipoDocumento> list) {
        this.list = list;
    }
    ...
    public Object getValueAt(int rowIndex, int columnIndex) {
        TipoDocumento c = list.get(rowIndex);
        switch (columnIndex) {
            case 0:
                return c.getNombre();
    ...
    public void atualizar(int row, TipoDocumento tipoDocumento) {
        list.set(row, tipoDocumento);
        this.fireTableRowsUpdated(row, row);
    }

    public TipoDocumento getCliente(int row) {
        return list.get(row);
    }
