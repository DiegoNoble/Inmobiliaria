...
public interface IGastoInmuebleInquilinoDAO extends JpaRepository<GastoInmuebleInquilino, Long> {
    ...
    @Query("select c from GastoInmuebleInquilino c where c.inquilino = ?1 and c.rubro.nombre like '%Otros gastos%' and c.situacion = ?2")
    List<GastoInmuebleInquilino> findOtrosGastosInquilinos(Inquilino inquilino, Situacion situacion);
