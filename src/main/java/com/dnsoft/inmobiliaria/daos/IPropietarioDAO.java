...
public interface IPropietarioDAO extends JpaRepository<Propietario, Long> {
    ...
    @Query("select new Propietario(p.id,p.nombre, p.documento, p.activo, p.direccion, p.tel, p.cel) from Propietario p where p.nombre like %?1% or p.documento like %?1%")
