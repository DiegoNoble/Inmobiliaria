...
import com.dnsoft.inmobiliaria.models.CallesTableModel;
 ...
 * @author Diego Noble
 */
public class DetalleComodidades extends javax.swing.JDialog {

    IComodidadesDAO comodidadesDAO;

    ApplicationContext applicationContext;
    Comodidades comodidad;
    List<Comodidades> listComodidades;
    ComodidadesTableModel tableModel;
    ListSelectionModel listModel;

    public DetalleComodidades(java.awt.Frame parent, boolean modal, Component frame) {
        super(parent, modal);
        initComponents();
    ...
    void configuraTbl() {
        ...
        listComodidades = new ArrayList<Comodidades>();
        listComodidades.addAll(comodidadesDAO.findAll());

        tableModel = new ComodidadesTableModel(listComodidades);
        ...
        tbl.addKeyListener(new KeyListener() {
            ...
            public void keyReleased(KeyEvent e) {
                ...
                if (keyCode == KeyEvent.VK_DOWN) {
                    ...
                    System.out.println(listComodidades.size());
                    if (tbl.getSelectedRow() == listComodidades.size() - 1) {
                        if (!listComodidades.get(tbl.getSelectedRow()).isNew()) {
                            comodidad = new Comodidades();
                            tableModel.agregar(comodidad);
                        }
                    }
                }
            }
    ...
    void elminarSeleccionado() {
        try {
            Comodidades toDelete = tableModel.getCliente(tbl.getSelectedRow());
            comodidadesDAO.delete(toDelete);
            actualizaTbl();
            btnEliminar.setEnabled(false);
    ...
    private void txtBuscaPropiedadActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_txtBuscaPropiedadActionPerformed
        ...
        listComodidades.clear();
        listComodidades.addAll(comodidadesDAO.findComodidades(txtBuscaPropiedad.getText()));
        tableModel.fireTableDataChanged();
